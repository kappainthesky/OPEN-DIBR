...
cleanup() {
    echo ""
    echo "[Launcher] Stopping Eye Tracker..."
    if [ -n "$EYE_PID" ]; then
...
# 1. Launch eye_3d_screen in background (connects to OpenDIBR RGB-D bridge on 127.0.0.1:9998)
...
./build/RealtimeDIBR --realsense -j examples/realsense/single_camera_config.json --auto_triangle_margin "$@"
