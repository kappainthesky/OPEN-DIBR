...
public:
	...
	Options(int argc, char* argv[]) {
		...
		for (size_t i = 0; i < normalizedPath.length(); ++i) {
			...
			if (normalizedPath[i] >= 'A' && normalizedPath[i] <= 'Z') {
				normalizedPath[i] = normalizedPath[i] - 'A' + 'a';
			}
		}
		if (normalizedPath.find("/examples/test/") != std::string::npos ||
			(normalizedPath.length() >= 6 && normalizedPath.substr(normalizedPath.length() - 6) == "/test/")) {
			// Only apply automatically if the user hasn't explicitly supplied --triangle_deletion_margin
